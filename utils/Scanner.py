...
class Scanner:
    ...
    def patterns_scanner(self,directory_path):
        ...
        if self.root_pattern_count != 0:
            cp.pr("info", f"[INFO] {self.root_pattern_count} Pattern/s Detected.")
            return True
        else:
                ...
                for file_name in files:
                    ...
                    if file_path.endswith(".smali"):
                        with open(file_path, 'r') as file:
                            ...
                            # Root Detection
                            self.root_detection_bypass(file_content)

                            # Emulator Detection
                            self.emulator_detection_bypass(file_content)
